...
def insert_employer(new_data: tuple, con):
    ...
    print(con)
    cur = con.cursor()
    cur.execute('INSERT INTO staff (name, last_name, position, salary, bonus) VALUES(?,?,?,?,?)', new_data)
    con.commit()
...
def select_last_name(con, last_name):
    cur = con.cursor()
    res = cur.execute(f'SELECT * FROM staff WHERE last_name LIKE "{last_name.capitalize()}%";').fetchall()
    return res
